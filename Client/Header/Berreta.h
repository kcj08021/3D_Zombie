#ifndef __BERETTA_H__
#define __BERETTA_H__

#include "Weapon.h"

BEGIN(Client)

class CBeretta final:
	public CWeapon{
private:
	enum ANIMATION_SET{
		IDLE				   = 0,
...
private:
	// 생성자
	explicit CBeretta(LPDIRECT3DDEVICE9 _pGraphicDevice);
	// 복사 생성자
	explicit CBeretta(const CBeretta& _Beretta);
	// 소멸자
	virtual ~CBeretta() = default;
...
public:
	// 원본 생성 함수(static)
	static CBeretta* CreateProto(LPDIRECT3DDEVICE9 _pGraphicDevice);
	// 복제 생성 함수
