...
CM4M203_Grenade::CM4M203_Grenade(LPDIRECT3DDEVICE9 _pGraphicDevice)
	: CWeapon(_pGraphicDevice)
	, m_pBullletBool(nullptr){
}

CM4M203_Grenade::CM4M203_Grenade(const CM4M203_Grenade & _m4m203)
	: CWeapon(_m4m203)
	, m_pBullletBool(nullptr){
}

// Initialize with Prototype
HRESULT CM4M203_Grenade::InitComponent_Proto(){
	...
	// 1인칭 모델 컴포넌트 추가
	m_pFirstPersonModelCom = dynamic_cast<CDynamicMesh*>(pManagement->AddCloneComponent(L"viewmodel_m4m203_grenade_Proto", STAGE_SCENE));
...
HRESULT CM4M203_Grenade::InitComponent_Clone(void * _pArgument){
	return CWeapon::InitComponent_Clone(_pArgument);
}

HRESULT CM4M203_Grenade::LateInitWeapon(){
	if(!m_bIsThirdPerson)
		m_pFirstPersonModelCom->SetUpAnimationSet(BULLET_TO_GRENADE, 1.5f);

	return NOERROR;
}

HRESULT CM4M203_Grenade::UpdateWeapon(CPicker* _pPicker, _double _dDeltaTime){
	if(!m_bIsThirdPerson){
		if(!m_bIsLateInit){
...
HRESULT CM4M203_Grenade::RenderWeapon(){
	return CWeapon::RenderWeapon();;
}

HRESULT CM4M203_Grenade::WeaponControl(CPicker* _pPicker){
	...
	if(eCurrentSet == IDLE){
		if(pManagement->IsKeyDown(DIK_5)){
			if(nullptr != m_pPlayerFlag)
				*m_pPlayerFlag &= ~CPlayer::FLAG_ADS;
			m_pFirstPersonModelCom->SetUpAnimationSet(GRENADE_TO_BULLET, 1.5f);
		}
	}
...
HRESULT CM4M203_Grenade::UpdateWeaponState(_double _dDeltaTime){
	ANIMATION_SET eCurrentSet = static_cast<ANIMATION_SET>(m_pFirstPersonModelCom->GetCurrentAnaimationSet());

	switch(eCurrentSet){
	case CM4M203_Grenade::IDLE:
		return NOERROR;
		break;
	case CM4M203_Grenade::FIRE:
		if(m_pFirstPersonModelCom->IsFinished()){
			--m_iCurrentMagazine;
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		}
		break;
	case CM4M203_Grenade::RELOAD:
		if(m_pFirstPersonModelCom->IsFinished()){
			CWeapon::Reload();
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		}
		break;
	case CM4M203_Grenade::GRENADE_TO_BULLET:
		if(m_pFirstPersonModelCom->IsFinished()){
			*m_pBullletBool = false;
			m_pFirstPersonModelCom->SetUpAnimationSet(BULLET_TO_GRENADE, 1.5f);
		}
		break;
	case CM4M203_Grenade::BULLET_TO_GRENADE:
		if(m_pFirstPersonModelCom->IsFinished()){
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		}
		break;
	case CM4M203_Grenade::SPRINT_IN:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(SPRINT_LOOP, 1.5f);
		break;
	case CM4M203_Grenade::SPRINT_LOOP:
		break;
	case CM4M203_Grenade::SPRINT_OUT:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		break;
	case CM4M203_Grenade::PULLOUT:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		break;
	case CM4M203_Grenade::PUTAWAY:
		if(m_pFirstPersonModelCom->IsFinished()){
		...
		break;
	case CM4M203_Grenade::QUICK_PULLOUT:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		break;
	case CM4M203_Grenade::QUICK_PUTAWAY:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(IDLE, 1.5f);
		break;
	case CM4M203_Grenade::KNIFE_MELEE1:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(QUICK_PULLOUT, 1.5f);
		break;
	case CM4M203_Grenade::KNIFE_MELEE2:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(QUICK_PULLOUT, 1.5f);
		break;
	case CM4M203_Grenade::NVG_PUTON:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(PULLOUT, 1.5f);
		break;
	case CM4M203_Grenade::NVG_TAKEOFF:
		if(m_pFirstPersonModelCom->IsFinished())
			m_pFirstPersonModelCom->SetUpAnimationSet(PULLOUT, 1.5f);
		break;
	default:
...
CM4M203_Grenade * CM4M203_Grenade::CreateProto(LPDIRECT3DDEVICE9 _pGraphicDevice){
	CM4M203_Grenade* pInstance = new CM4M203_Grenade(_pGraphicDevice);
	if(FAILED(pInstance->InitComponent_Proto())){
		SafeRelease(pInstance);
		MSG_BOX(L"CM4M203_Grenade Create Failure");
	}

	return pInstance;
}

CComponent * CM4M203_Grenade::CreateClone(void * _pArgument){
	CComponent* pInstance = new CM4M203_Grenade(*this);
